...
static pthread_cond_t bridge_open;
...
int main (int argc, char *argv[]) {
    ...
    //cond_init(&bridge_open), USYNC_THREAD, (void *) NULL);
    pthread_cond_init(&(bridge_open), NULL);
...
void *function(void *vehicles) {
    ...
    if(current_data->queue_flag==false) {
            ...
            pthread_cond_wait(&bridge_open, &bridge);
    ...
    for(;;) {
        ...
        if(current_data->queue_flag==true) {
            ...
            pthread_cond_signal(&bridge_open);    
            pthread_mutex_unlock(&bridge);
